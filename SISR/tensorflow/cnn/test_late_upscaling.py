...
input = tf.placeholder(tf.float32, (1, downscaled_image.shape[0], downscaled_image.shape[1], params.num_channels), name='input') 
output = nets.SRCNN_late_upscaling(input, params.kernel_size)

if(params.num_channels == 1):
    downscaled_image = np.expand_dims(downscaled_image, 3)
