...
# training 
batch_size = 128 
input = tf.placeholder(tf.float32, (batch_size, data_reader.dim_patch_in_2,  data_reader.dim_patch_in, params.num_channels), name='input')
target_2 = tf.placeholder(tf.float32, (batch_size, data_reader.dim_patch_gt, data_reader.dim_patch_gt, params.num_channels), name='target_2')
...
for epoch in range(start_epoch, params.num_epochs):
	...
	for i in range(0, num_iterations): 
		 ...
		 cost, _, predicted_images = sess.run([loss, opt, output], feed_dict={input: input_ , target_2: input_4, target: target_, starter_learning_rate: lr})
