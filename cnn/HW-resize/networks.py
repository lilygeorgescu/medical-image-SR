...
def SRCNN_late_upscaling(im, kernel_size, num_layers = params.layers): 
    ...
    # first layer
	output = tf.layers.conv2d(im, filters = 32, kernel_size = 5, strides = 1, padding='SAME', activation=tf.nn.relu, kernel_regularizer = tf.contrib.layers.l2_regularizer(scale=reg))
    
	output = tf.layers.conv2d(output, filters = 32, kernel_size = 5, strides = 1, padding='SAME', activation=tf.nn.relu, kernel_regularizer = tf.contrib.layers.l2_regularizer(scale=reg))	  
