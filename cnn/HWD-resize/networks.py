...
def SRCNN_late_upscaling_D(im, kernel_size, num_layers = params.layers): 
    ...
    feature_map_for_ps = tf.layers.conv2d(output, filters = params.num_channels * params.scale, kernel_size = 3, strides = 1, padding='SAME', activation=tf.nn.relu, kernel_regularizer = tf.contrib.layers.l2_regularizer(scale=reg))  
    
    output__ = PS_D(feature_map_for_ps, params.scale)
    
    output_ = tf.layers.conv2d(output__, filters = params.num_channels, kernel_size = 3, strides = 1, padding='SAME', activation = None, kernel_regularizer = tf.contrib.layers.l2_regularizer(scale=reg))
    
    return output_, feature_map_for_ps, output__