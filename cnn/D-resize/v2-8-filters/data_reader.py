...
class DataReader:
    ...
    def __init__(self, train_path, eval_path, test_path, is_training=True, SHOW_IMAGES=False): 
        ...
        if(is_training):
            self.train_images_in = np.concatenate((utils.read_all_patches_from_directory(train_path, 'input%s' % params.dim_patch), utils.read_all_patches_from_directory(train_path, 'input%s' % params.dim_patch_2), utils.read_all_patches_from_directory(train_path, 'input_orig_14_14')))
            ...
            self.train_images_gt = np.concatenate((utils.read_all_patches_from_directory(train_path, 'gt%s' % params.dim_patch), utils.read_all_patches_from_directory(train_path, 'gt%s' % params.dim_patch_2), utils.read_all_patches_from_directory(train_path, 'gt_orig_14_14')))
