...
def SRCNN_late_upscaling_D(im, reuse=False, is_training=False): 
 	...
 	with tf.name_scope('depth_net') as scope:  
            ...
            output_1 = tf.layers.conv2d(im, filters=8, kernel_size=3, strides=1, padding='SAME', activation=tf.nn.relu, reuse=reuse, name='depth_net/conv2d')	  
     
        # residual block
            output_2 = tf.layers.conv2d(output_1, filters=8, kernel_size=3, strides=1, padding='SAME', activation=tf.nn.relu, reuse=reuse, name='depth_net/conv2d_1') 
            output_3 = tf.layers.conv2d(output_2, filters=8, kernel_size=3, strides=1, padding='SAME', activation=tf.nn.relu, reuse=reuse, name='depth_net/conv2d_2')
            output_4 = tf.add(tf.multiply(output_1 , 1), output_3)
            if is_training:
                output_4 = tf.nn.dropout(output_4, keep_prob=0.9)
            output = tf.layers.conv2d(output_4, filters=8, kernel_size=3, strides=1, padding='SAME', activation=tf.nn.relu, reuse=reuse, name='depth_net/conv2d_3')
            output = tf.layers.conv2d(output, filters=8, kernel_size=3, strides=1, padding='SAME', activation=tf.nn.relu, reuse=reuse, name='depth_net/conv2d_4')     
        ...
        # print(feature_map_for_ps.shape , output_1.shape)
            ...
            output_5 = tf.layers.conv2d(output_PS, filters=8, kernel_size=3, strides=1, padding='SAME', activation=tf.nn.relu, name='depth_net/last_layer_1', reuse=reuse)
         # residual block
            if is_training:
                output_5 = tf.nn.dropout(output_5, keep_prob=0.9)
            output_6 = tf.layers.conv2d(output_5, filters=8, kernel_size=3, strides=1, padding='SAME', activation=tf.nn.relu, name='depth_net/last_layer_2', reuse=reuse)
            output_7 = tf.layers.conv2d(output_6, filters=8, kernel_size=3, strides=1, padding='SAME', activation=tf.nn.relu, name='depth_net/last_layer_3', reuse=reuse)
